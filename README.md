...
this is fully functional redis high availabe pods with initcontainer to change sysctl config as tcp backlog need somaxconn to be higher than 120 that is default of alpine base image , another change is nodeselector for running redis on nodes wich labeled as ```redis=ture``` that makes your work easy in private clusters.
i removed volume mount and persistent storage from template but if you need persist data you can easly add from [kubernetes official example](https://github.com/kubernetes/examples/tree/master/staging/storage/redis)
